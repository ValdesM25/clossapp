      ...
      {isCartOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-white z-50 flex flex-col shadow-2xl border-l border-zinc-200"
          >
            {/* Header */}
            <div className="p-4 border-b border-zinc-100 flex items-center justify-between bg-zinc-900 text-white">
              <div className="flex items-center gap-2">
                <ShoppingBag className="w-5 h-5 text-emerald-400" />
                <h2 className="font-serif text-lg text-white">Tu Carrito de Compras</h2>
                {itemCount > 0 && (
            ...
            <div className="bg-emerald-50 border-b border-emerald-100 p-3.5 flex items-start gap-3">
              ...
              {cart.length === 0 ? (
                ...
                cart.map((item) => (
                  <motion.div
                    ...
                    className="flex gap-3 p-3 border border-zinc-200 rounded-lg bg-zinc-50/50 relative group"
                  >
                    <div className="w-20 h-24 rounded bg-zinc-200 overflow-hidden shrink-0 border border-zinc-200">
                      <img
                    ...
                    <div className="flex-1 flex flex-col justify-between min-w-0 pr-6">
                      <div>
                        <div className="flex items-center gap-1.5 mb-1">
                          <span
                            className={cn(
                              "text-[10px] uppercase tracking-wider px-2 py-0.5 font-semibold rounded",
                              item.tipo === "renta"
                          ...
                          {item.prenda.talla && (
                            <span className="text-[10px] text-zinc-500 bg-zinc-200 px-1.5 py-0.5 rounded">
                              Talla {item.prenda.talla}
                            </span>
                          )}
                        </div>
                        <h4 className="font-serif text-sm text-zinc-900 truncate">{item.prenda.name}</h4>
                        ...
                        <p className="text-xs text-zinc-500 mt-0.5">Vendedor: {item.sellerName || "Verificado"}</p>
                      </div>
                      <div className="flex items-center justify-between mt-2 pt-1 border-t border-zinc-200/60">
                        <span className="font-mono text-sm font-bold text-zinc-900">${item.precio} MXN</span>
                      </div>
                    </div>
                    <button
            ...
            {cart.length > 0 && (
              <div className="p-4 border-t border-zinc-200 bg-zinc-50 space-y-3">
                ...
                <div className="flex gap-2 pt-1">
                  <button
                    onClick={clearCart}
                    className="px-3 py-2.5 text-xs text-zinc-500 hover:text-zinc-800 border border-zinc-300 rounded transition-colors"
                  >
                    Vaciar
                  </button>
                  <motion.button
